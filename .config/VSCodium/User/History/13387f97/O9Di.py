...
def difference_biggest_relative(data_f):
    ...
    """
    tmp_data_f = data_f.loc[(data_f["Gold"] > 0) & (data_f["Gold.1"] > 0)].loc[:, ["Gold", "Gold.1", "Gold.2", "ID"]]

    return pd.Series.abs(
        (tmp_data_f["Gold"] - tmp_data_f["Gold.1"]) / tmp_data_f["Gold.2"]
    ).idxmax()
...
def get_points(data_f):
    ...
    """
    data_f["Points"] = (
        data_f["Gold.2"] * 3 + data_f["Silver.2"] * 2 + data_f["Bronze.2"]
    )
