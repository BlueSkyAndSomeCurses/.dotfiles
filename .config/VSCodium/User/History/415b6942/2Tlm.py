...
def possible_moves(field, piece, player):
    lst_of_coordinates = []
    counter = 0
...
def opponent_coordinates(field, player):
...
def closest_move(opponent_coordinates, possible_moves):
    min_distance = 1000000000000000000
    selected_move = possible_moves[0]

    for move in possible_moves:
        for opponent_coord in opponent_coordinates:
...
# def player_move(current_field: list, coordinates: list, player: int) -> list:
...
def step(player: int):
    ...
    debug(f"Coordinates: {coor_of_moves}")
    a = opponent_coordinates(field, player)
    move = closest_move(a, coor_of_moves)
    # move = player_move(field, coor_of_moves, player)
