...
def closest_move(oppent_coords: list, possible_moves_: list) -> tuple:
    """
    ...
    for move in possible_moves_:
        for opponent_coord in oppent_coords:
            distance = (move[0] - opponent_coord[0])**2 + (move[1] - opponent_coord[1])**2
...
def step(player: int):
    ...
    for row, line in enumerate(piece):
        for x, chr_ in enumerate(line):
            if chr_ == "*":
                piece_coords.append((row))
