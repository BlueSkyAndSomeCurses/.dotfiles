...
def closest_move(oppent_coords: list, possible_moves_: list, piece: list, player) -> tuple:
    """
            ...
            for x, char in enumerate(line):
                if char == ".":
                    for opponent_coord in oppent_coords:
                        distance = (move[0] + y - opponent_coord[0]) ** 2 + (
                            move[1] + x - opponent_coord[1]
                        ) ** 2
...
def step(player: int):
    ...
    move = closest_move(coords_opponent, coor_of_moves, piece)
    debug(move)
